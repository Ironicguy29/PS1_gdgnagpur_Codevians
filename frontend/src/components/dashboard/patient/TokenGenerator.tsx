...
// Fallback demo data for offline / backend-unavailable scenarios
const DEMO_DOCTORS = [
...
export function TokenGenerator({ patientId, onTokenGenerated }: TokenGeneratorProps) {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      // Augment with live queue_length from /queue/live if possible
      const enriched = await Promise.all(rawDocs.map(async (doc: any) => {
        ...
        } catch { return { ...doc, queue_length: 0, avg_wait: 15 }; }
      }));
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const token = res.data?.token?.display_token || res.data?.token_number || res.data?.token?.token_number;
      const doctorId = selectedDoc._id;
      ...
      onTokenGenerated?.(token, doctorId);
    } catch (err: any) {
      // Demo fallback — show a simulated token so the UI still works
